...
class Initor_for_btn(QtWidgets.QMainWindow):
    ...
    def init_right(self):
        ...
        self.mask_video.setText("等待视频检测")
        self.mask_video.setFixedSize(
            self.video_size[0], self.video_size[1])  # width height
        self.mask_video.setStyleSheet("QLabel{background:white;}"
                                      "QLabel{color:rgb(100,100,100);"
                                      "font-size:15px;"
                                      "font-weight:bold;font-family:宋体;}"
                                      "border-radius: 25px;border: 1px solid black;")
        ...
        self.right_bar_layout.addWidget(self.mask_video, 1, 0)
    ...
    def init_left(self):
        ...
        self.left_button_2 = QtWidgets.QPushButton(
            qtawesome.icon('fa.play', color='white'), "打开摄像头")
...
class Initor_for_event(Initor_for_btn):
    ...
    def init_btn_event(self):
        ...
        # self.left_reset.clicked.connect(QApplication.processEvents)
